...
export class EventsService {
    ...
    async getFutureEventWithWorkshops() {
        const resul = await this.eventRepository.query('select * from events left join events.id=workshop.eventId where cast(workshop.createdAt as date) > cast(now() as date) ')
        return resul
    }
}
